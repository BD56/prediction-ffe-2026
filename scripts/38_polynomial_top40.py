"""
38 - Régression polynomiale degré 2 (avec interactions) sur top 40 features.
...
Sortie : data/master/polynomial_top40_results.csv
"""
...
def main():
    print("=== 37 - Polynomial degré 2 (avec interactions) sur top 40 ===\n")

    # ---------- 1. Top 20 features ----------
    imp = pd.read_csv(MASTER_DIR / "top_flop_v2_avec_valeurs.csv")
    top40 = imp.sort_values("rank_moyen").head(40)["feature"].tolist()
    print(f"Top 40 features sélectionnées (rang moyen consensus v2).")
    ...
    for label, res in [
        ...
        ("ElasticNet top 40 (linéaire)", res_lin),
        ("ElasticNet top 40 + Poly deg2 + interact.", res_poly),
    ]:
    ...
    print("-" * 105)
    for label, tr in [
        ("ElasticNet top 40 (linéaire)", tr_lin),
        ("ElasticNet top 40 + Poly deg2 + interact.", tr_poly),
    ]:
    ...
    pd.DataFrame(rows).to_csv(MASTER_DIR / "polynomial_top40_results.csv", index=False)

    # ---------- 7. Quelles interactions sont gardées ? ----------
    feat_names = poly.get_feature_names_out([f"x{i+1}" for i in range(40)])
    ...
    import re
    name_map = {f"x{i+1}": top40[i] for i in range(40)}
    def replace_names(t):
    ...
    coefs.to_csv(MASTER_DIR / "polynomial_top40_coefs.csv", index=False)
